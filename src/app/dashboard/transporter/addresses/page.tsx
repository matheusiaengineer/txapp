...
import {
  MapPin, Plus, Pencil, Trash2, Star, X, Save, Navigation,
...
const iconMap: Record<string, any> = { home: Home, work: Building2, other: MapPin };
...
export default function TransporterAddressesPage() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setPlaces((data || []).map((a: any) => ({
          id: a.id,
          name: a.type === "home" ? "Base" : a.type === "work" ? "Terminal" : a.full_address.split(",")[0],
  ...
  const handleAddPlace = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        setPlaces([{ id: data.id, name: newPlace.name || data.full_address.split(",")[0], address: data.full_address, icon: MapPin, color: "#3ECB8E", isFavorite: false }, ...places]);
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-4 sm:py-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="space-y-3">
            <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wider">Endereços salvos ({places.length})</h2>
              ...
              {places.map((place) => {
                ...
                return (
                  <motion.div key={place.id} layout initial={{ opacity: 0, scale: 0.95 }}
                    ...
                    {isEditing ? (
                      <div className="space-y-3">
                        <input type="text" defaultValue={place.name} placeholder="Nome do local" id={`tedit-${place.id}`}
                        ...
                        <input type="text" defaultValue={place.address} placeholder="Endereço completo" id={`tedit-addr-${place.id}`}
                          className="w-full bg-background border border-card-border rounded-xl px-4 py-2.5 text-sm text-white placeholder:text-gray-500 outline-none focus:border-primary/50" />
                        <div className="flex gap-2">
                          <motion.button whileTap={{ scale: 0.97 }}
                            onClick={() => {
                              const n = (document.getElementById(`tedit-${place.id}`) as HTMLInputElement)?.value || place.name;
                              const a = (document.getElementById(`tedit-addr-${place.id}`) as HTMLInputElement)?.value || place.address;
                              handleUpdate(place.id, n, a);
                            }}
                          ...
                          <motion.button whileTap={{ scale: 0.97 }} onClick={() => setEditingId(null)}
                            className="px-4 py-2 text-sm text-gray-400 hover:text-foreground">Cancelar</motion.button>
                        </div>
                      </div>
                    ) : (
            ...
            {places.length === 0 && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass-panel p-10 text-center">
                ...
                <p className="text-xs text-gray-600 mt-1">Adicione seus pontos de carga e descarga</p>
              </motion.div>
            )}
          </div>
          ...
          <div className="space-y-6">
            <AnimatePresence>
              {showAddForm && (
                <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="glass-panel p-5">
                  ...
                  <div className="space-y-3">
                    <input type="text" placeholder="Nome (ex: Galpão, Porto, Cliente)" value={newPlace.name}
                    ...
                    <input type="text" placeholder="Endereço completo" value={newPlace.address}
                      onChange={(e) => setNewPlace({ ...newPlace, address: e.target.value })}
                      className="w-full bg-background border border-card-border rounded-xl px-4 py-3 text-sm text-white placeholder:text-gray-500 outline-none focus:border-primary/50 transition-colors" />
                    <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={handleAddPlace}
                      className="w-full bg-primary text-background font-semibold py-3 rounded-xl text-sm">Salvar endereço</motion.button>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
            ...
            <div className="glass-panel p-5">
              ...
              <ul className="space-y-2 text-sm text-gray-400">
                <li className="flex items-start gap-2"><Truck className="w-4 h-4 text-primary mt-0.5 shrink-0" /><span>Salve terminais e clientes frequentes para agilizar fretes</span></li>
                <li className="flex items-start gap-2"><Star className="w-4 h-4 text-yellow-400 mt-0.5 shrink-0" /><span>Favoritos aparecem como sugestão ao criar novo frete</span></li>
                <li className="flex items-start gap-2"><Scale className="w-4 h-4 text-blue-400 mt-0.5 shrink-0" /><span>Adicione balanças e postos de pesagem no trajeto</span></li>
              </ul>
            </div>
          </div>
        </div>
