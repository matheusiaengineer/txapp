      ...
      <div className="max-w-4xl mx-auto px-4 py-6 space-y-5">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between">
          ...
          <button onClick={() => setShowInvite(!showInvite)}
            className="bg-primary hover:bg-primary-hover text-background text-xs font-bold px-4 py-2.5 rounded-xl flex items-center gap-1.5 transition-all">
        ...
        <AnimatePresence>
          {showInvite && (
            <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="glass-panel p-5 space-y-3">
              ...
              <button className="w-full py-2 bg-primary text-background text-xs font-bold rounded-xl hover:bg-primary-hover transition-all">
          ...
          {drivers.map(driver => (
            <motion.div key={driver.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              ...
              <div className="flex items-center gap-2">
                ...
                <button className="p-1.5 hover:bg-white/5 rounded-lg"><MoreHorizontal className="w-4 h-4 text-gray-400" /></button>
              </div>
            </motion.div>
          ))}
        </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-panel p-5">
          ...
          <div className="grid grid-cols-3 gap-3 text-center">
            ...
            <div className="bg-background rounded-xl p-3">
              ...
              <p className="text-[10px] text-gray-500">Gasto hoje</p>
            </div>
          </div>
        </motion.div>
      </div>
