...
export default function EmployeeDashboard() {
  ...
  const [orders] = useState<any[]>([
    { id: "PED-001", client: "João Silva", status: "pending", items: "2 pizzas", createdAt: "10 min atrás" },
    { id: "PED-002", client: "Maria Souza", status: "in_transit", items: "1 bolo, 3 refrigerantes", createdAt: "25 min atrás" },
    { id: "PED-003", client: "Carlos Lima", status: "delivered", items: "5 marmitex", createdAt: "1h atrás" },
  ]);
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <div className="flex items-center justify-between mb-1">
            <h1 className="text-2xl font-bold">
              <span className="text-primary">Meu Painel</span>
            </h1>
              ...
              <div key={order.id} className="glass-panel p-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                    ...
                    order.status === "in_transit" ? "bg-blue-400/10" : "bg-yellow-400/10"
                  }`}>
                    {order.status === "delivered" ? <Check className="w-5 h-5 text-primary" /> :
                     order.status === "in_transit" ? <Clock className="w-5 h-5 text-blue-400" /> :
                ...
                <div className="text-right">
                  ...
                  <p className="text-[10px] text-gray-600 mt-1">{order.createdAt}</p>
                </div>
              </div>
            );
          })}
