      ...
      >
        <div className="text-center mb-8">
          <Link href="/" className="inline-flex items-center gap-2 mb-6">
            ...
            <span className="text-2xl font-bold text-white">TXD</span>
          </Link>
        ...
        <div className="glass-panel p-8">
          ...
          <div className="flex bg-background rounded-xl p-1 mb-6 border border-card-border">
            {methods.map(m => (
              <button
                key={m.id}
          ...
          <form onSubmit={handleLogin} className="space-y-4">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full bg-background border border-card-border rounded-lg p-3 pr-12 text-white focus:border-primary focus:outline-none transition-colors"
                      placeholder="••••••••"
                      required
                    />
            ...
            {method === "phone" && (
              <div className="flex flex-col gap-2">
                ...
                <div className="flex gap-2">
                  <div className="bg-background border border-card-border rounded-lg px-3 flex items-center text-gray-400 text-sm">
                    {country.phoneCode}
                  </div>
                  <input
                    type="tel"
                ...
                <p className="text-xs text-gray-500 mt-1">Login por telefone em breve</p>
              </div>
            )}
            ...
            {method === "biometric" && (
              <div className="text-center py-8">
                ...
                {biometricAvailable ? (
                  <>
                    <p className="text-gray-400 text-sm mb-2">Toque para entrar com biometria</p>
                    <p className="text-xs text-gray-500 mb-4">Face ID, Touch ID ou impressão digital</p>
                    <button
                    ...
                    >
                      {biometricLoading ? (
                        ...
                        <><Fingerprint className="w-5 h-5" /> Entrar com Face ID</>
                      )}
                    </button>
                  </>
                ) : (
                  <>
                    <p className="text-gray-400 text-sm mb-2">Biometria não disponível</p>
                    <p className="text-xs text-gray-500">Seu dispositivo não suporta ou você ainda não cadastrou</p>
                    <div className="mt-4 bg-primary/5 border border-primary/20 rounded-xl p-3 text-xs text-gray-400">
                      Faça login com email e nas configurações ative a biometria.
                    </div>
                  </>
                )}
              </div>
            ...
            <AnimatePresence>
              {error && (
                <motion.div initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }}
                  className="flex items-center gap-2 bg-red-500/10 border border-red-500/30 rounded-lg p-3 text-sm text-red-400">
            ...
            >
              {loading ? (
                ...
                <>Em breve <ArrowRight className="w-5 h-5" /></>
              ) : method === "biometric" ? null : (
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center"><span className="bg-[#1c1c1c] px-4 text-sm text-gray-500">ou continue com</span></div>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <button disabled
              className="flex items-center justify-center gap-2 py-3 bg-background border border-card-border rounded-xl opacity-50 cursor-not-allowed text-sm font-medium">
          ...
          <p className="text-center text-xs text-gray-600">Login social em breve</p>
        </div>
