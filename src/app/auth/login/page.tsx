...
function LoginForm() {
  ...
  const [success] = useState(searchParams.get("conta") === "criada")
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    router.push(roleRoute[role] || `/dashboard/${role}`)
  }
    ...
    <main className="min-h-[100dvh] flex flex-col bg-background">
      <div className="flex-1 flex flex-col px-6 max-w-sm mx-auto w-full pt-12 pb-8">
        <div className="text-center mb-10">
          ...
          <h1 className="text-2xl font-bold text-foreground">Entrar no TXAP</h1>
          <p className="text-sm text-muted mt-1">Bem-vindo de volta!</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          {success && (
            <div className="flex items-center gap-2 text-success text-sm bg-success/10 border border-success/20 px-4 py-3 rounded-2xl">
              ...
              <span>Conta criada! Faca login para continuar.</span>
            </div>
          )}
          {error && (
        ...
        <button
          ...
          disabled={loading}
          className="w-full border border-card-border hover:bg-card-bg-2 text-foreground font-medium py-3.5 rounded-2xl flex items-center justify-center gap-3 transition-all active:scale-[0.98]"
        >
